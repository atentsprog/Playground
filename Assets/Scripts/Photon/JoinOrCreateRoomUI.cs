...
public class JoinOrCreateRoomUI : MonoBehaviourPunCallbacks
{
    ...
    {
        Debug.Log("<Color=Red>OnJoinRandomFailed</Color>: Next -> Create a new Room" + returnCode + message);
    }
    ...
    public override void OnJoinedRoom()
    {
        ...
        PhotonNetwork.Instantiate(tankPrefabName, new Vector3(
            Random.Range(-randomRange, randomRange)
            , 0
            , Random.Range(-randomRange, randomRange)
            ), Quaternion.identity, 0);
    }
    public string tankPrefabName = "Tank";
